    ...
    () => {
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onComplete: () => {
            playingRef.current = false;
            // Restore element to a plain text node so CSS background-clip:text
            // (gradient text) works — GSAP's wrapped spans break that inheritance.
            el.textContent = text;
            wrappersRef.current = [];
            try { splitRef.current?.revert(); } catch {}
            splitRef.current = null;
            ...
            onShuffleComplete?.();
            if (loop) {
              // Re-trigger after loopDelay so the text is fully restored (gradient
              // visible) during the pause, then shuffles again.
